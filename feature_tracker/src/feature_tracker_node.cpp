...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    time stamp
    string frame_id
    geometry_msgs/Point32[] points        点云坐标点，所有角点的坐标（x,y,1)
    float32 x
    float32 y
    float32 z
    sensor_msgs/ChannelFloat32[] channels  通道信息，放像素点2D(u,v)，像素的速度（vx,vy)
    string name
    float32[] values
    */
   if (PUB_THIS_FRAME)
